export class FrameMetrics {
  ...
  record(now, sample = true) {
    if (sample && this.last !== null) { this.samples[this.cursor] = now - this.last; this.cursor = (this.cursor + 1) % this.samples.length; this.count = Math.min(this.count + 1, this.samples.length); }
    this.last = now;
